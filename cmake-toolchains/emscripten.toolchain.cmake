...
if (FIPS_EMSCRIPTEN_USE_SAFE_HEAP)
    ...
    set(EMSC_LINKER_FLAGS "${EMSC_LINKER_FLAGS} -s SAFE_HEAP=0")
endif()
...
if (FIPS_EMSCRIPTEN_USE_DFE)
    ...
    set(EMSC_LINKER_FLAGS "${EMSC_LINKER_FLAGS} -s ELIMINATE_DUPLICATE_FUNCTIONS=0")
    set(EMSC_LINKER_FLAGS "${EMSC_LINKER_FLAGS} -s ELIMINATE_DUPLICATE_FUNCTIONS_DUMP_EQUIVALENT_FUNCTIONS=0")
endif()
if (FIPS_COMPILE_VERBOSE)
    set(EMSC_LINKER_FLAGS "${EMSC_LINKER_FLAGS} -s VERBOSE=1")
else()
    set(EMSC_LINKER_FLAGS "${EMSC_LINKER_FLAGS} -s VERBOSE=0")
endif()
...
if (FIPS_EMSCRIPTEN_USE_WASM)    
    set(EMSC_LINKER_FLAGS "${EMSC_LINKER_FLAGS} -s BINARYEN=1 -s 'BINARYEN_METHOD=\"native-wasm\"' -s 'BINARYEN_SCRIPTS=\"spidermonkify.py\"' -s GLOBAL_BASE=1000 -s ALIASING_FUNCTION_POINTERS=0")
elseif(FIPS_EMSCRIPTEN_USE_WASM_ASMJS)
    set(EMSC_LINKER_FLAGS "${EMSC_LINKER_FLAGS} -s BINARYEN=1 -s 'BINARYEN_METHOD=\"native-wasm,asmjs\"' -s 'BINARYEN_SCRIPTS=\"spidermonkify.py\"' -s GLOBAL_BASE=1000 -s ALIASING_FUNCTION_POINTERS=0")
endif()
set(EMSCRIPTEN_EXE_LINKER_FLAGS "${EMSC_EXE_LINKER_FLAGS} --shell-file ${FIPS_EMSCRIPTEN_SHELL_HTML}")
