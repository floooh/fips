...
def finish(sdk_dir) :
    ...
    if config.get_host_platform() == 'win' :
        ...
        subprocess.call(args=['emsdk.bat', 'install', 'sdk-incoming-64bit'], cwd=sdk_dir, shell=True)
        subprocess.call(args=['emsdk.bat', 'activate', 'sdk-incoming-64bit'], cwd=sdk_dir, shell=True)
    else :
        subprocess.call(args=['./emsdk update'], cwd=sdk_dir, shell=True)
        subprocess.call(args=['./emsdk install sdk-incoming-64bit'], cwd=sdk_dir, shell=True)
        subprocess.call(args=['./emsdk activate sdk-incoming-64bit'], cwd=sdk_dir, shell=True)
